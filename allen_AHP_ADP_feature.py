...
def AHP_amplitude_1st_save():
	...
	AHP_idx_array=np.append(AHP_idx_array,[])/(10*t)
...
def AHP_latency_1st_save():
	...
	threshold_idx_array=np.append(threshold_idx_array,[])/(10*t)
	index1=len(threshold_idx_array)
	if len(AHP_idx_array)==len(threshold_idx_array):
		for n in range(0,index1):
			AHP_latency=abs(AHP_idx_array[n]-threshold_idx_array[n])*t
			spike_index='spike'+'_'+str(n+1)
...
def ADP_save():
	...
	next_begin_idx=np.append(next_begin_idx,threshold_idx_array[index1-1]+mean_ISI/t)
	if len(AHP_idx_array)==len(threshold_array):
		for n in range(0,index1):
			...
			if ADP_idx!=0:
				...
				AHP_latency_2nd.loc[sweep_id+idx7,spike_index]=abs(second_AHP_idx-threshold_idx_array[n])*t
			
				delta_AHP_latency.loc[sweep_id+idx8,spike_index]=abs(second_AHP_idx-AHP_idx_single)*t
			
				ADP_latency.loc[sweep_id+idx9,spike_index]=abs(ADP_idx-threshold_idx_array[n])*t
			else:
